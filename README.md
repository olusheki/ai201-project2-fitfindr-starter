...
Information from one tool get transferred to the next within the entire `run_agent` function in `agent.py`. It starts by always searching the listings for a possible item, and storing results in the session dictonary. Then, if the output is suitable to proceed, it calls another tool and accesses the necessary information from the session dictionary.

It's tracking the most relevant item, the error state (if it ran into any), the parsed user query, the search results, the outfit suggestion, and the fit card.
...
|------|-------------|----------------|
| search_listings | No results match the query | "Sorry, I couldn't find any items that matched your description. Please try again or ask for something else." and quits.|
| suggest_outfit | Wardrobe is empty | returns a string with general styling advice for the item, not the whole wardrobe |
| create_fit_card | Outfit input is missing or incomplete | Says "Please enter a search query" |
...
I said: "Are there any bugs of things to look out for in run_agent?"

It responded with: "Three things to fix or watch out for:

1. Line 140 — the error stub needs to be removed, not just commented out.
You return the session right after, which is correct, but the commented line is dead weight. Delete it.

2. Line 119 — json.loads will crash if the LLM adds extra text around the JSON.
LLMs sometimes prefix the response with something like "Here is the JSON:" even when told not to. Wrap it in a try/except:
...
3. Line 135 — you pass wardrobe directly but the session already stores it as session["wardrobe"].
These are the same value here so it works, but for consistency it's cleaner to use session["wardrobe"] so everything flows through session state as designed.
...
"You've found a great thrifted item. The Graphic Tee — 2003 Tour Bootleg Style is a versatile piece that can be styled in various ways to achieve different looks. Here are some general styling advice and tips:

**Vibe:** This tee suits a grunge, streetwear, or alternative vibe, perfect for those who want to add an edgy touch to their outfit. It's also great for music fans, especially those who appreciate the nostalgia of a 2003 tour tee.

**Pairing suggestions:**

1. **Distressed denim:** Pair the tee with distressed, high-waisted, or ripped jeans for a classic grunge look.
2. **Leather jackets:** Add a biker or denim jacket to give your outfit a cool, edgy vibe.
3. **Skirts:** For a more feminine touch, pair the tee with a flowy skirt or a pleated mini skirt.
4. **Shorts:** Wear the tee with black shorts and sneakers for a casual, summer look.
5. **Hoodies or sweatshirts:** Layer the tee under a hoodie or sweatshirt for a relaxed, streetwear-inspired outfit.

**Footwear:**

1. **Sneakers:** Converse, Vans, or Adidas sneakers will complement the tee's streetwear vibe.
2. **Boots:** Pair the tee with ankle boots, such as Dr. Martens or combat boots, for a grungier look.
3. **Sandals:** For a more laid-back look, wear the tee with sandals or flip-flops.

**Accessories:**

1. **Choker necklaces:** Add a choker necklace to give your outfit a '90s-inspired touch.
2. **Hats:** Wear a beanie, trucker hat, or a baseball cap to complete the streetwear look.
3. **Belt bags:** Use a belt bag or a fanny pack to add a functional, streetwear-inspired element to your outfit.
"
